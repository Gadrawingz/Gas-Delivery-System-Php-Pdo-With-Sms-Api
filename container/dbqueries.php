...
class GasDelivery {
	...
	public function register_station_manager($uname, $email, $phone, $pass) {
		$sql = "INSERT INTO `station_manager`(`m_id`, `username`, `email`, `password`, `phone_number`) VALUES (null, '$uname', '$email', '".$pass."', '$phone')";
	...
	public function register_customer_self($names, $uname, $phone, $address, $pin) {
		$sql= "INSERT INTO `customers`(`cust_id`, `cust_names`, `username`, `phone_number`, `address`, `pin_number`) VALUES (null, '$names', '$uname', '$phone', '$address', '".$pin."')";
		$query= $this->con->prepare($sql);
		$query->execute();
		$count= $query->rowCount();
		return $count;
	...
	public function register_agent($names, $uname, $phone, $address, $pass) {
		$sql = "INSERT INTO `station_agent`(`agent_id`, `agent_names`, `username`, `phone_number`, `address`, `password`) VALUES (null, '$names', '$uname', '$phone', '$address', '".$pass."')";
		$query= $this->con->prepare($sql);
		$query->execute();
		$count= $query->rowCount();
		return $count;
	...
	public function register_customer($names, $uname, $phone, $address) {
		$sql= "INSERT INTO `customers`(`cust_id`, `cust_names`, `username`, `phone_number`, `address`, `pin_number`) VALUES (null, '$names', '$uname', '$phone', '$address', '".rand(1000, 9999)."')";
		$query= $this->con->prepare($sql);
		$query->execute();
		$count= $query->rowCount();
		return $count;
	...
	public function update_customer($names, $uname, $phone, $address, $id) {
		$sql= "UPDATE `customers` SET cust_names='$names', username='$uname', phone_number='$phone', address= '$address' WHERE cust_id='$id' ";
		$query= $this->con->prepare($sql);
		$query->execute();
		$count= $query->rowCount();
		return $count;
	...
	public function submit_gas($quantity, $comment, $cid, $aid) {
		$sql= "INSERT INTO `gas_store`(`g_id`, `gas_quantity`, `comment`, `cust_id`, `agent_id`, `status`, `date_submitted`) VALUES (null, '$quantity', '$comment', '$cid', '$aid', 'Unpaid', NOW() )";
		$query= $this->con->prepare($sql);
		$query->execute();
		$count= $query->rowCount();
		return $count;
